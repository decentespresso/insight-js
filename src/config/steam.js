...
export const steamConfig = {
  ...
  elements: [
    ...
    // ---- auto-off clicker (jug dial): + top / - bottom (also on the done page) ----
    { kind: 'button', pages: ['steam_1', 'steam_3'], rect: [200, 560, 900, 855], action: 'adjust',
      adj: { key: 'steamDuration', delta: +1, min: 1, max: 255, set: (v) => ({ steamSettings: { duration: v } }) } },
    { kind: 'button', pages: ['steam_1', 'steam_3'], rect: [200, 855, 900, 1150], action: 'adjust',
      adj: { key: 'steamDuration', delta: -1, min: 1, max: 255, set: (v) => ({ steamSettings: { duration: v } }) } },
    row(['steam_1', 'steam_3'], 536, 1238, 'center', C.val, (l) => `${n0(l.steamDuration)} ${t('seconds')}`, { size: F.val, weight: 'bold' }),
    row(['steam_1', 'steam_3'], 536, 1292, 'center', C.val, () => t('AUTO-OFF'), { size: F.cap, spacing: 2 }),
    // tap the value -> full-screen numeric entry
    { kind: 'button', pages: ['steam_1', 'steam_3'], rect: [200, 1198, 900, 1320], action: 'numpad',
    ...
    // ---- settings block: Enabled (functional toggle) / Preheat / Temperature / Flow
    // rate (also on the done page so it stays editable after steaming) ----
    row(['steam_1', 'steam_3'], 1096, 1166, 'nw', C.slabel, () => t('Enabled'), { size: F.set }),
    { kind: 'box', pages: ['steam_1', 'steam_3'], rect: [1600, 1150, 1706, 1196], bg: '#5b9bd8', radius: 23,
      bind: (l) => ({ bg: l.steamEnabled === false ? '#c3c7d4' : '#5b9bd8' }) },
    { kind: 'box', pages: ['steam_1', 'steam_3'], rect: [1662, 1152, 1704, 1194], bg: '#ffffff', radius: 21,
      bind: (l) => ({ left: l.steamEnabled === false ? 1602 : 1662 }) },
    { kind: 'button', pages: ['steam_1', 'steam_3'], rect: [1560, 1142, 1720, 1204], action: 'toggleSteam' },
    ...
    row(['steam_1', 'steam_3'], 1096, 1218, 'nw', C.slabel, () => t('Preheat to'), { size: F.set }),
    row(['steam_1', 'steam_3'], 1690, 1218, 'ne', C.sval, (l) => (l.steamEnabled === false ? t('off') : `${fmtTemp(l.steamTarget, 0)}`), { size: F.set }),
    row(['steam_1', 'steam_3'], 1096, 1270, 'nw', C.slabel, () => t('Temperature'), { size: F.set }),
    row(['steam_1', 'steam_3'], 1690, 1270, 'ne', C.sval, (l) => `${fmtTemp(l.steamTemp > 0 ? l.steamTemp : l.steamTarget)}`, { size: F.set }),
    row(['steam_1', 'steam_3'], 1096, 1318, 'nw', C.slabel, () => t('Flow rate max'), { size: F.set }),
    row(['steam_1', 'steam_3'], 1690, 1318, 'ne', C.sval, (l) => `${n1(l.steamFlowMax)} mL/s`, { size: F.set }),
    ...
    row(['steam', 'steam_3'], 1840, 250, 'nw', C.title, () => t('Information'), { size: F.title, weight: 'bold' }),
    row(['steam', 'steam_3'], 1840, 360, 'nw', C.slabel, () => t('Steaming'), { size: F.set }),
    row(['steam', 'steam_3'], 2500, 360, 'ne', C.sval, (l) => `${n0(l.runElapsed)} ${t('seconds')}`, { size: F.set }),
    row(['steam', 'steam_3'], 1840, 430, 'nw', C.slabel, () => t('Auto-Off'), { size: F.set }),
    row(['steam', 'steam_3'], 2500, 430, 'ne', C.sval, (l) => `${n0(l.steamDuration)} ${t('seconds')}`, { size: F.set }),
    row(['steam', 'steam_3'], 1840, 500, 'nw', C.slabel, () => t('Temperature'), { size: F.set }),
    row(['steam', 'steam_3'], 2500, 500, 'ne', C.sval, (l) => `${fmtTemp(l.steamTemp, 0)}`, { size: F.set }),
    row(['steam', 'steam_3'], 1840, 570, 'nw', C.slabel, () => t('Pressure (bar)'), { size: F.set }),
    row(['steam', 'steam_3'], 2500, 570, 'ne', C.sval, (l) => `${n1(l.pressure)}`, { size: F.set }),
    row(['steam', 'steam_3'], 1840, 640, 'nw', C.slabel, () => t('Flow rate max'), { size: F.set }),
    row(['steam', 'steam_3'], 2500, 640, 'ne', C.sval, (l) => `${n1(l.steamFlowMax)} mL/s`, { size: F.set }),
    row(['steam', 'steam_3'], 1840, 710, 'nw', C.slabel, () => t('Flow rate now'), { size: F.set }),
    row(['steam', 'steam_3'], 2500, 710, 'ne', C.sval, (l) => `${n1(l.flow)} mL/s`, { size: F.set }),

    // ---- live pressure / flow / temperature mini graph — small, bottom-aligned to
    // the lower-right card (Tcl: 700x300 at 1810,1090; card bottom ~1391) ----
    ...
    { kind: 'slider', pages: ['steam_1', 'steam', 'steam_3'], rect: [10, 1436, 2000, 1586], handleW: 500, trough: '#d5d9e8', fill: '#f7f5ff',
