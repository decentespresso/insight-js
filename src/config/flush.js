...
export const flushConfig = {
  ...
  elements: [
    // ---- card titles ----
    row(['preheat_1'], 70, 250, 'nw', C.title, () => `1) ${t('Choose auto-off time')}`, { size: F.title, weight: 'bold' }),
    row(['preheat_1'], 1056, 250, 'nw', C.title, () => `2) ${t('Hot water will pour')}`, { size: F.title, weight: 'bold' }),
    ...
    row(['preheat_1'], 70, 370, 'nw', '#5a5d75', (l) => (l.flushSeconds > 10 && l.flushFlowMax > 4.0 ? t('Warning: long flush times can cool the group head') : ''), { size: 32 }),
    ...
    { kind: 'button', pages: ['preheat_1'], rect: [290, 865, 750, 1160], action: 'adjust',
      adj: { key: 'flushSeconds', delta: -1, min: 3, max: 120, set: (v) => ({ rinseData: { duration: v } }) } },
    row(['preheat_1'], 520, 1238, 'center', C.val, (l) => `${n0(l.flushSeconds)} ${t('seconds')}`, { size: F.val, weight: 'bold' }),
    row(['preheat_1'], 520, 1292, 'center', C.val, () => t('AUTO-OFF'), { size: F.cap, spacing: 2 }),
    ...
    row(['preheat_1'], 1394, 768, 'center', C.wait, () => t('START'), { size: F.wait, weight: 'bold' }),
    row(['preheat_2'], 1394, 768, 'center', C.wait, () => t('STOP'), { size: F.wait, weight: 'bold' }),
    row(['preheat_3', 'preheat_4'], 1394, 768, 'center', C.wait, () => t('RESTART'), { size: F.wait, weight: 'bold' }),
    row(['preheat_1', 'preheat_2', 'preheat_3', 'preheat_4'], 1394, 856, 'center', C.mode, () => t('Flush').toUpperCase(), { size: F.mode }),
    ...
    row(['preheat_2'], 1840, 1136, 'nw', C.slabel, () => t('Flushing'), { size: F.set }),
    row(['preheat_2'], 2500, 1136, 'ne', C.sval, (l) => `${n0(l.runElapsed)} ${t('seconds')}`, { size: F.set }),
    row(['preheat_2'], 1840, 1206, 'nw', C.slabel, () => t('Flow rate'), { size: F.set }),
    row(['preheat_2'], 2500, 1206, 'ne', C.sval, (l) => `${n1(l.flow)} mL/s`, { size: F.set }),
    row(['preheat_2'], 1840, 1276, 'nw', C.slabel, () => t('Auto-Off'), { size: F.set }),
    row(['preheat_2'], 2500, 1276, 'ne', C.sval, (l) => `${n0(l.flushSeconds)} ${t('seconds')}`, { size: F.set }),
