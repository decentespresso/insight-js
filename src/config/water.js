...
export const waterConfig = {
  ...
  elements: [
    // ---- card titles ----
    row(['water_1'], 70, 250, 'nw', C.title, () => `1) ${t('Choose quantity and temperature')}`, { size: F.title, weight: 'bold' }),
    row(['water_1'], 1056, 250, 'nw', C.title, () => `2) ${t('Hot water will pour')}`, { size: F.title, weight: 'bold' }),
    ...
    row(['water_1'], 300, 1292, 'center', C.val, () => t('Volume').toUpperCase(), { size: F.cap, spacing: 2 }),
    ...
    row(['water_1'], 1394, 768, 'center', C.wait, () => t('START'), { size: F.wait, weight: 'bold' }),
    row(['water'], 1394, 768, 'center', C.wait, () => t('STOP'), { size: F.wait, weight: 'bold' }),
    row(['water_3'], 1394, 768, 'center', C.wait, () => t('RESTART'), { size: F.wait, weight: 'bold' }),
    row(['water_1', 'water', 'water_3'], 1394, 856, 'center', C.mode, () => t('Water').toUpperCase(), { size: F.mode }),
    ...
    row(['water'], 1394, 980, 'center', C.title, (l) => `${n0(l.weight)} g`, { size: 44 }),
    ...
    row(['water'], 1840, 1118, 'nw', C.slabel, () => t('Pouring'), { size: F.set }),
    row(['water'], 2500, 1118, 'ne', C.sval, (l) => `${n0(l.runElapsed)} ${t('seconds')}`, { size: F.set }),
    row(['water'], 1840, 1188, 'nw', C.slabel, () => t('Temperature'), { size: F.set }),
    row(['water'], 2500, 1188, 'ne', C.sval, (l) => `${fmtTemp(l.mixTemp, 0)}`, { size: F.set }),
    row(['water'], 1840, 1258, 'nw', C.slabel, () => t('Volume'), { size: F.set }),
    row(['water'], 2500, 1258, 'ne', C.sval, (l) => `${n0(l.waterVolume)} mL`, { size: F.set }),
    row(['water'], 1840, 1328, 'nw', C.slabel, () => t('Flow rate'), { size: F.set }),
    row(['water'], 2500, 1328, 'ne', C.sval, (l) => `${n1(l.flow)} mL/s`, { size: F.set }),
  ],
};
